  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        console.error("Navbar: product fetch error", err);
      }
    };
    fetchAll();
  }, []);
                ...
                {suggestions.map((product) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-black uppercase italic truncate">
                        {product.name}
                      </p>
        ...
        <div className="navbar-end gap-1 md:gap-2">
          {user && (
            <div className="hidden md:flex items-center gap-1 mr-2 bg-base-200/50 p-1 rounded-xl border border-base-300">
              <Link
                to="/"
                className="btn btn-ghost btn-xs h-8 px-3 gap-2 font-black uppercase italic text-[9px] tracking-[0.15em] hover:bg-base-100 rounded-lg transition-all"
              >
          ...
          {!isAdmin && user && (
            <div className="dropdown dropdown-hover dropdown-end">
              <div
                tabIndex={0}
                role="button"
                className="btn btn-ghost btn-circle btn-sm relative transition-all hover:bg-primary/10"
              >
                <ShoppingBag size={20} />
                {uniqueCartCount > 0 && (
                ...
                <div className="card card-compact w-80 bg-base-100 shadow-2xl border border-base-200 rounded-2xl overflow-hidden">
                  <div className="card-body p-0">
                    <div className="p-4 border-b border-base-200 bg-base-200/30 text-center">
                      <span className="text-[10px] font-black uppercase italic opacity-50 tracking-widest">
                        Bag Summary ({uniqueCartCount} Items)
                      </span>
                    </div>
                    {uniqueCartCount > 0 ? (
                      <>
                        <div className="max-h-[300px] overflow-y-auto scrollbar-thin scrollbar-thumb-primary">
                          {cart?.items?.map((item, idx) => (
                            <div
                              key={idx}
                              className="flex gap-3 items-center p-3 border-b border-base-200/50 group hover:bg-base-200/20"
                            >
                              <img
                                src={
                                ...
                                alt=""
                                className="w-10 h-10 object-cover rounded bg-base-200"
                              />
                              <div className="flex-1 min-w-0">
                        ...
                        <div className="p-3">
                          <Link
                            to="/dashboard/cart"
                            className="btn btn-primary btn-sm w-full rounded-xl font-black uppercase italic shadow-md"
                          >
                            Checkout Now
                          </Link>
                        </div>
              ...
              <ul className="menu p-2 shadow-2xl bg-base-100 rounded-box w-48 border border-base-200">
                {themes.map((t) => (
                  <li key={t}>
                    <button
                      onClick={() => setTheme(t)}
                      className={`capitalize flex justify-between ${theme === t ? "bg-primary/10 text-primary font-bold" : ""}`}
                    >
          ...
          <div key={user?._id || "guest"} className="flex items-center">
            {user ? (
              <div className="dropdown dropdown-hover dropdown-end">
                <label
                  tabIndex={0}
                  className="btn btn-ghost btn-circle avatar border-2 border-primary/20 hover:border-primary ml-1 transition-all overflow-hidden p-0"
                >
                  <div className="w-8 md:w-9 rounded-full overflow-hidden bg-base-300 flex items-center justify-center">
                    <img
                      src={
                        user.profilePic?.url ||
                        `https://ui-avatars.com/api/?name=${user.username}&background=641ae6&color=fff&bold=true`
                      }
                      alt={user.username}
                ...
                <div tabIndex={0} className="dropdown-content z-[60] pt-2">
                  <ul className="menu p-3 shadow-2xl bg-base-100 rounded-2xl w-64 border border-base-200">
                    ...
                    <li className="md:hidden">
                      <Link to="/" className="font-bold text-[11px] uppercase">
                    ...
                    {(isAdmin ? adminLinks : userLinks).map((link) => (
                      <li key={link.path}>
                        <Link
                          to={link.path}
                          className={`py-2 px-3 text-[11px] font-bold uppercase tracking-tight hover:bg-base-200 rounded-lg flex items-center gap-3 group ${location.pathname === link.path ? "text-primary bg-primary/5" : ""}`}
                        >
                    ...
                    <li className="mt-1">
                      ...
                      >
                        ...
                        <ChevronDown
                          size={12}
                          className={`transition-transform duration-300 ${isNavSettingsOpen ? "rotate-180" : ""}`}
                        />
                      </button>
                      <div
                        className={`overflow-hidden transition-all duration-300 ${isNavSettingsOpen ? "max-h-40 opacity-100 mb-2" : "max-h-0 opacity-0"}`}
                      >
                    ...
                    <li>
                      <button
                        ...
                        className="text-error font-black uppercase text-[10px] py-3 tracking-widest hover:bg-error/10 flex justify-center border border-dashed border-error/20 rounded-xl mt-1"
                      >
                        Logout Session
                      </button>
                    </li>
              ...
              <div className="flex gap-1 ml-2">
                <Link
                  to="/login"
                  className="btn btn-ghost btn-sm px-4 font-black uppercase text-[10px]"
                >
                  Login
                </Link>
                <Link
                  to="/register"
                  className="btn btn-primary btn-sm px-4 font-black uppercase text-[10px] shadow-lg shadow-primary/20"
                >
                  Join
                </Link>
              </div>
