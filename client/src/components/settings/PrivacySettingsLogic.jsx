...
import { Trash2, AlertTriangle, KeyRound, Eye, EyeOff, ShieldCheck, X, AlertCircle, Loader2 } from "lucide-react";
...
const PrivacySettingsLogic = () => {
  ...
  const isOverLimit = passwordData.current.length > LIMITS.password || passwordData.new.length > LIMITS.password;
  ...
  const LoadingOverlay = ({ message }) => (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="flex flex-col items-center bg-base-100 p-8 rounded-[32px] border border-white/10 shadow-2xl">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.2em] italic opacity-70">{message}</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative animate-in fade-in duration-500">
      {/* GLOBAL OVERLAYS */}
      {(loading && !showOtpModal) && <LoadingOverlay message="Processing Request..." />}
      {sendingDeleteCode && <LoadingOverlay message="Generating Security Code..." />}
      {(loading && showOtpModal) && <LoadingOverlay message="Verifying Identity..." />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-base-200/50 border border-base-300 rounded-[32px] p-8">
          <div className="flex items-center gap-4 mb-8">
             ...
             <h3 className="font-black uppercase italic tracking-tight text-base-content">Security Credentials</h3>
          </div>
          <form onSubmit={handleRequestUpdate} className="space-y-4">
            <div>
              <div className="flex justify-between text-[10px] font-black uppercase opacity-40 mb-2">
                ...
                <span className={passwordData.current.length > LIMITS.password ? "text-error" : ""}>
            ...
            <div>
              <div className="flex justify-between text-[10px] font-black uppercase opacity-40 mb-2">
                ...
                <span className={passwordData.new.length > LIMITS.password ? "text-error" : ""}>
            ...
            <button disabled={isOverLimit || loading} className={`btn btn-primary w-full rounded-xl uppercase italic mt-4 ${isOverLimit ? 'btn-error' : ''}`}>
              {loading ? <span className="loading loading-spinner loading-sm"></span> : isOverLimit ? "Limit Exceeded" : "Request Update"}
            </button>
          </form>
        </div>
        ...
        <div className="bg-error/5 border border-error/10 rounded-[32px] p-8 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-4 mb-4">
              ...
              <h3 className="font-black uppercase italic text-error">Danger Zone</h3>
            </div>
      ...
      {/* OTP MODAL */}
      {showOtpModal && (
        <div className="modal modal-open backdrop-blur-md" onClick={(e) => e.target === e.currentTarget && !loading && setShowOtpModal(false)}>
          <div className="modal-box bg-base-100 border border-primary/20 rounded-[40px] p-10 text-center relative shadow-2xl">
            <button onClick={() => setShowOtpModal(false)} className="btn btn-sm btn-circle btn-ghost absolute right-6 top-6"><X size={20}/></button>
            <div className="w-16 h-16 bg-primary/10 text-primary rounded-2xl flex items-center justify-center mx-auto mb-6">
              <ShieldCheck size={32} />
            </div>
            <h3 className="text-xl font-black uppercase italic mb-2">Verify Identity</h3>
            <p className="text-[10px] font-bold uppercase opacity-50 mb-6">Enter the 6-digit code sent to your email</p>
            <input 
              type="text" maxLength="6" placeholder="000000" disabled={loading}
            ...
            />
            <button onClick={handleVerifyOtp} disabled={otp.length !== 6 || loading} className="btn btn-primary w-full rounded-2xl h-14 font-black uppercase italic">
      ...
      <dialog id="delete_modal" className="modal">
        <div className="modal-box rounded-[32px] border border-error/20 p-8">
          <h3 className="text-xl font-black text-error uppercase italic flex items-center gap-2"><AlertCircle size={24} /> Final Warning</h3>
          <p className="py-4 text-sm font-bold opacity-60 leading-relaxed">Verification required. A code will be sent to your Gmail.</p>
          <div className="modal-action flex gap-3">
            <form method="dialog" className="flex-1"><button className="btn btn-ghost w-full rounded-xl font-black uppercase">Cancel</button></form>
            <button onClick={handleRequestDeleteCode} disabled={sendingDeleteCode} className="btn btn-error flex-[2] rounded-xl font-black uppercase italic">
              {sendingDeleteCode ? <span className="loading loading-spinner"></span> : "Send Deletion Code"}
            </button>
          </div>
        </div>
      </dialog>
