  ...
  return (
    <div className="modal modal-open backdrop-blur-sm p-2 md:p-4">
      ...
      <div className="modal-box max-w-4xl p-0 bg-base-100 border border-base-300 shadow-2xl overflow-y-auto md:overflow-hidden relative rounded-2xl h-auto max-h-[95vh] md:max-h-[85vh]">
        ...
        <button
          className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 z-50 bg-base-100/50 backdrop-blur-md"
          onClick={onClose}
        >
          <X className="w-5 h-5" />
        </button>
          ...
          <div className="w-full md:w-[55%] p-5 md:p-8 flex flex-col justify-between bg-base-100">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {product.sizes?.map((s) => (
                    <button
                      ...
                      `}
                    >
                      <span className="text-xs md:text-sm font-black italic uppercase">
                        {s.size}
                      </span>
                      {s.stock > 0 && (
                        <span
                          className={`text-[8px] font-bold mt-0.5 uppercase ${s.stock < 5 ? "text-error" : "opacity-40"}`}
                        >
                          {s.stock} Left
                        </span>
                      )}
            ...
            <div className="space-y-3 mt-4">
              {!user ? (
                <Link
                  to="/login"
                  className="btn btn-primary w-full h-12 rounded-xl font-black uppercase italic shadow-lg shadow-primary/20"
                >
                  Login to Buy
                </Link>
              ) : isSoldOut ? (
                <button
                  disabled
                  className="btn btn-ghost w-full h-12 rounded-xl font-black uppercase italic border-2 border-base-300 gap-2"
                >
