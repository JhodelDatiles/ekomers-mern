...
const Checkout = () => {
  ...
  // ─────────────────────────────────────────────────────────────────
  const startPollingIntent = useCallback(
    (paymentIntentId) => {
      let attempts = 0;
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          const result =
            await paymentAPI.getOrderByPaymentIntent(paymentIntentId);
          ...
          if (attempts >= 2) {
            const { status } =
              await paymentAPI.checkQrPhStatus(paymentIntentId);
            if (status === "succeeded") {
              clearInterval(interval);
              setPollingId(null);
              // Call confirmQrPhOrder to update the pending order in DB
              paymentAPI
                .confirmQrPhOrder({ paymentIntentId })
                .then(() => {
                  console.log("✅ Order confirmed via fallback");
                })
                .catch((err) => {
                  console.error(
  ...
  const handleQrPhPayment = async () => {
    ...
    console.log(
      "🔍 selectedCartItemIds to send:",
      checkoutItems.map((i) => i._id),
    );
    ...
    try {
      const result = await paymentAPI.createQrPhPayment({
        ...
        directProductId: directItem
          ? directItem.productId?._id || directItem.productId
          : undefined,
        directQuantity: directItem?.quantity,
        directSize: directItem?.size,
        selectedCartItemIds: !isDirectPurchase
          ? checkoutItems.map((i) => String(i._id))
          : undefined,
      });
  ...
  useEffect(() => {
    if (user?.address?.length > 0) {
      const stillExists = user.address.find(
        (a) => a._id === activeAddress?._id,
      );
    ...
    <div className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in fade-in duration-700">
        ...
        <div className="relative h-40 w-full bg-[#121212] rounded-[32px] border border-white/5 overflow-hidden flex items-center px-10">
          ...
          <div className="relative z-10 flex flex-col gap-2 w-full">
            <div className="flex justify-between items-start">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-3 h-3 rounded-full transition-all duration-500 ${isSyncing ? "bg-blue-500 scale-125" : "bg-primary animate-pulse"}`}
                  />
            ...
            <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest flex items-center gap-2">
              <Navigation size={10} /> Sector:{" "}
              {activeAddress?.city || "Unspecified Sector"}
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#121212] border border-white/5 rounded-[24px] p-8">
          ...
          <div className="flex flex-wrap gap-3">
            {["Barangay", "City", "Postal Code"].map((tag, i) => (
              <span
                key={tag}
                className="bg-white/5 px-4 py-2 rounded-xl text-[10px] font-black text-white/40 border border-white/5"
              >
                {tag}:{" "}
                <span className="text-white">
                  {[
                    activeAddress?.barangay,
                    activeAddress?.city,
      ...
      <div className="lg:col-span-4">
        <div className="sticky top-24 bg-primary text-black rounded-[40px] p-10 shadow-2xl">
          <h3 className="font-black uppercase italic text-2xl mb-3 border-b-2 border-black/10 pb-4">
            Order Details
          </h3>
          ...
          <div className="space-y-6 mb-10 max-h-[120px] overflow-y-auto pr-2 custom-manifest-scrollbar">
            {checkoutItems.map((item) => (
              <div
                key={item._id}
                className="flex justify-between items-start font-bold uppercase text-[11px]"
              >
                <div className="flex flex-col w-2/3">
                  ...
                  {item.size && (
                    <span className="text-[9px] opacity-70 mt-1 font-black italic">
                      SIZE: {item.size}
                    </span>
                  )}
                </div>
          ...
          <div className="border-t-2 border-black/10 pt-8 mb-8">
            <span className="text-[10px] font-black uppercase opacity-40 mb-1 block">
              Total Amount
            </span>
          ...
          <div className="bg-[#121212] border border-white/10 w-full max-w-lg rounded-[40px] overflow-hidden shadow-2xl">
              ...
              {user?.address?.map((addr, idx) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <span
                      className={`text-[9px] font-black uppercase px-3 py-1 rounded-full ${activeAddress?._id === addr._id ? "bg-primary text-black" : "bg-white/10 text-white/60"}`}
                    >
                      {addr.label}
                    </span>
                    {activeAddress?._id === addr._id && (
            ...
            <div className="p-8 bg-black/40 border-t border-white/5">
              <button
                ...
                className="w-full py-4 border-2 border-dashed border-white/10 rounded-2xl text-[10px] font-black uppercase text-white/40 hover:border-primary hover:text-primary transition-all"
              >
