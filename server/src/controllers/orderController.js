...
export const initiatePayMongoCheckout = async (req, res) => {
  try {
    ...
    const isDirect =
      isDirectPurchase ||
      (items[0]?._id && String(items[0]._id).startsWith("direct"));

    const metadata = {
      userId: String(req.user.id),
      cartItemIds: isDirect
        ? "DIRECT_BUY"
        : items.map((item) => item._id).join(","),
      directItemData: isDirect
        ? JSON.stringify(
            items.map((item) => ({
    ...
    res
      .status(200)
      .json({ checkoutUrl: response.data.data.attributes.checkout_url });
  } catch (error) {
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    const canAutoRefund =
      ...
      AUTO_REFUNDABLE.includes(order.paymentMethod);
    ...
    if (canAutoRefund) {
      try {
        ...
        if (paymentId) {
          ...
          console.log(
            `💸 PayMongo refund triggered: ${refundId} for order ${order._id}`,
          );
        } else {
          console.warn(
            `⚠️ Could not find payment ID for order ${order._id} — refund skipped`,
          );
        }
      } catch (refundErr) {
        console.error(
          `❌ PayMongo refund failed for order ${order._id}:`,
      ...
      console.log(
        `⚠️ Manual refund required for order ${order._id} (method: ${order.paymentMethod}, amount: ₱${order.totalAmount})`,
      );
    }

    // ── 3. UPDATE ORDER ──
    order.status = "Cancelled";
    order.paymentStatus = refundId
      ? "refunded"
      : needsManualRefund
        ? "refund_pending"
        : order.paymentStatus;
    ...
    console.log(
      `✅ Order ${order._id} cancelled. Reason: "${reason}". Refund: ${refundId || (needsManualRefund ? "MANUAL REQUIRED" : "N/A")}`,
    );
